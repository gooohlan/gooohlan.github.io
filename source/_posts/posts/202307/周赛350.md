...
- 如果付费刷，那么花费的时间要加上 $time[i]$
- 如果免费刷，那么花费的时间需要减一(因为两个油漆匠可以同时进行)

因此，定义 $dp[i][j]$ 为刷前 $i$ 堵墙，耗时为 $j$  的最小开销，根据上述思路分类讨论：

- 付费刷第 $i$ 面墙：$dp[i+1][j+time[i]] = min(dp[i][j] = $
- 免费刷第 $i$ 面墙：$dp[i+1][j-1] = min(dp[i][j], dp[i+1][j-1])$

当 $j \geq n$ 时，后序所有的墙都可以免费刷，所有的 $j \geq n$ 与 $j = n$ 等效，对于每个 $dp[i]$ ，可以保留 $j = -n \sim n$ 总共 $2n+1$个状态
